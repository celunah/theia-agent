"""Compatibility entry point for Theia Agent."""

import asyncio

from theia.server.core import (
    CodexAppServer,
    CodexAppServerError,
    CodexTurnCancelled,
    CodexTurnTimeoutError,
)
from theia.server.policy import (
    CODEX_MEMORY_RESTART_BACKOFF_ENV,
    CODEX_STDIO_LIMIT_ENV,
    DEFAULT_CODEX_MEMORY_RESTART_BACKOFF,
    DEFAULT_CODEX_STDIO_LIMIT,
    MAX_CODEX_STDIO_LIMIT,
    MIN_CODEX_STDIO_LIMIT,
    SESSION_ARCHIVE_AFTER,
    SESSION_DELETE_AFTER,
)
from theia.core import (
    CodexTransientRestartError,
    _PendingApproval,
    _MoodState,
    _Session,
    _TurnState,
)
from theia.audio import (
    AudioOutput,
    AudioProtocolError,
    AudioServiceConfig,
    OpenAICompatibleAudio,
    TTSConfig,
)
from theia.bot.core import (
    CodexBot,
    TheiaBot,
    _about_embed,
    _credits_embed,
    _interaction_allows_tools,
    _interaction_request_sender,
    _is_guild_install,
    _is_server_admin,
    _is_user_only_install,
    _login_required_embed,
    _mention_prompt,
    _maybe_create_response_thread,
    _message_context,
    _name_new_response_thread,
    _personality_summary_embed,
    _restart_in_place,
    _should_respond_to_message,
    _typing_indicator,
    _usage_embed,
    _usage_details_embed,
    _UsageView,
    bot,
    codex_approve,
    codex_about,
    codex_btw,
    codex_credits,
    codex_customize,
    codex_deny,
    codex_improvements,
    codex_login,
    codex_mode,
    codex_memory,
    codex_model,
    codex_personality,
    codex_restart,
    codex_skill,
    codex_stop,
    codex_undo,
    codex_usage,
    customization_element_autocomplete,
    customization_target_autocomplete,
    handle_login,
    handle_request,
    model_autocomplete,
    personality_autocomplete,
    session_key,
    skill_autocomplete,
)
from theia.server.threads import (
    thread_name as _thread_name,
    user_requested_thread as _user_requested_thread,
)
from theia.server.codex_update import (
    CODEX_AUTO_UPDATE_ENV,
    CODEX_UPDATE_INTERVAL_ENV,
    CODEX_UPDATE_TIMEOUT_ENV,
    DEFAULT_CODEX_AUTO_UPDATE,
    DEFAULT_CODEX_UPDATE_INTERVAL,
    DEFAULT_CODEX_UPDATE_TIMEOUT,
    CodexUpdateResult,
    CodexUpdater,
)
from theia.server.lighthouse import LighthouseView, render_lighthouse
from theia.server.vault import VaultError
from theia.core import (
    ADAPTIVE_REASONING_ENV,
    AGENT_DISPLAY_NAME,
    AGENT_NAME,
    ALWAYS_ADMIN_USERS_ENV,
    APPROVAL_LEVEL_ENV,
    APPROVAL_LEVELS,
    BASE_PRIORS,
    CODEX_LOG_COLORS_ENV,
    DEFAULT_CODEX_MODEL,
    DEFAULT_APPROVAL_LEVEL,
    DEFAULT_MODE,
    DEFAULT_PERSONALITY_SCOPE,
    DEFAULT_NIGHTLY_RECAP_TIMEOUT,
    DEFAULT_REASONING_EFFORT,
    DEFAULT_SELF_IMPROVEMENT,
    DEFAULT_SELF_IMPROVEMENT_TIMEOUT,
    MOOD_BASELINE_STRENGTH,
    MOOD_DECAY_PER_MINUTE,
    MOOD_LABELS,
    PERSONALITY_SCOPES,
    NIGHTLY_RECAP_TIMEOUT_ENV,
    SELF_IMPROVEMENT_ENV,
    SELF_IMPROVEMENT_TIMEOUT_ENV,
    SUPER_ADMIN_USERS_ENV,
    TEXT_MODE,
    THEIA_VERSION,
    VOICE_MODE,
    _command_embed,
    _codex_logger,
    _configured_paths,
    _configured_user_ids,
    _env_bool,
    _env_float,
    _error_message,
    _is_tool_item,
    _is_always_admin_user,
    _is_super_admin_user,
    _path_from_value,
    _safe_approval_reason,
    _safe_error_reason,
    _safe_intermediate_text,
    _safe_log_label,
    _safe_thinking_status,
    _skill_entries,
    _subtext,
    _theia_revision,
    _truncate,
    _verified_change_status,
)
from theia.delivery import (
    SendMessage,
    _ImageResultView,
    _MemoryView,
    _PaginatorView,
    _reaction_paginators,
    _ResponseDelivery,
    _format_thought_duration,
    _split_pages,
    send_paginated,
    send_response,
)
from theia.personality import (
    PersonalityError,
    PersonalityProfile,
    PersonalityStore,
    PersonalitySummary,
)
from theia.recaps import (
    DEFAULT_NIGHTLY_RECAP,
    DEFAULT_NIGHTLY_RECAP_CONTEXT_MAX_CHARACTERS,
    DEFAULT_NIGHTLY_RECAP_TIMEZONE,
    MAX_NIGHTLY_RECAP_CONTEXT_MAX_CHARACTERS,
    NIGHTLY_RECAP_CONTEXT_MAX_ENV,
    NIGHTLY_RECAP_ENV,
    NIGHTLY_RECAP_TIMEZONE_ENV,
    NightlyRecapManager,
)
from theia.customization import (
    COMMAND_TARGETS,
    ELEMENTS,
    LABEL_TARGETS,
    PLACEHOLDERS,
    CustomizationError,
    FrontendCustomizationStore,
    canonical_target,
    customization_context,
    display_target,
    render_template,
)
from theia.presence import (
    DEFAULT_PRESENCE_IDLE_AFTER,
    DEFAULT_PRESENCE_LONG_TASK_AFTER,
    DEFAULT_PRESENCE_UPDATE_INTERVAL,
    DEFAULT_RICH_PRESENCE_ACTIVE_DEBOUNCE,
    DEFAULT_RICH_PRESENCE_CONTEXT_MAX_AGE,
    DEFAULT_RICH_PRESENCE_ENABLED,
    DEFAULT_RICH_PRESENCE_IDLE_INTERVAL,
    DEFAULT_RICH_PRESENCE_RECENT_IDLE_INTERVAL,
    DEFAULT_RICH_PRESENCE_TIMEOUT,
    PresenceManager,
    RichPresenceManager,
)
from theia.voice import (
    VOICE_FRAME_BYTES,
    VoiceConversationSink,
    VoiceModeError,
    VoiceModeManager,
    VoiceSegment,
    VoiceSession,
)
from theia.audio_provider import (
    AUDIO_PROVIDER_AUTO,
    AUDIO_PROVIDER_CODEX_REALTIME,
    AUDIO_PROVIDER_CUSTOM,
    AUDIO_PROVIDER_ENV,
    AUDIO_PROVIDER_EVENTS,
    AUDIO_PROVIDER_QWEN,
    AUDIO_PROVIDER_UNAVAILABLE,
    AudioProvider,
    AudioProviderCapabilities,
    AudioProviderError,
    AudioProviderEvent,
    AudioProviderEventCallback,
    QWEN_AUDIO_SEMANTIC_ENV,
    QWEN_AUDIO_TOKEN_ENV,
    QWEN_AUDIO_URL_ENV,
    QwenAudioProvider,
)
from theia.ui import (
    _DecisionView,
    _FormView,
    _JsonModal,
    _PromptModal,
    _TextModal,
    _UserInputView,
)

__all__ = [
    "ADAPTIVE_REASONING_ENV",
    "AGENT_DISPLAY_NAME",
    "AGENT_NAME",
    "ALWAYS_ADMIN_USERS_ENV",
    "APPROVAL_LEVELS",
    "APPROVAL_LEVEL_ENV",
    "AUDIO_PROVIDER_AUTO",
    "AUDIO_PROVIDER_CODEX_REALTIME",
    "AUDIO_PROVIDER_CUSTOM",
    "AUDIO_PROVIDER_ENV",
    "AUDIO_PROVIDER_EVENTS",
    "AUDIO_PROVIDER_QWEN",
    "AUDIO_PROVIDER_UNAVAILABLE",
    "BASE_PRIORS",
    "CODEX_AUTO_UPDATE_ENV",
    "CODEX_LOG_COLORS_ENV",
    "CODEX_MEMORY_RESTART_BACKOFF_ENV",
    "CODEX_STDIO_LIMIT_ENV",
    "CODEX_UPDATE_INTERVAL_ENV",
    "CODEX_UPDATE_TIMEOUT_ENV",
    "COMMAND_TARGETS",
    "DEFAULT_APPROVAL_LEVEL",
    "DEFAULT_CODEX_AUTO_UPDATE",
    "DEFAULT_CODEX_MEMORY_RESTART_BACKOFF",
    "DEFAULT_CODEX_MODEL",
    "DEFAULT_CODEX_STDIO_LIMIT",
    "DEFAULT_CODEX_UPDATE_INTERVAL",
    "DEFAULT_CODEX_UPDATE_TIMEOUT",
    "DEFAULT_MODE",
    "DEFAULT_NIGHTLY_RECAP",
    "DEFAULT_NIGHTLY_RECAP_CONTEXT_MAX_CHARACTERS",
    "DEFAULT_NIGHTLY_RECAP_TIMEOUT",
    "DEFAULT_NIGHTLY_RECAP_TIMEZONE",
    "DEFAULT_PERSONALITY_SCOPE",
    "DEFAULT_PRESENCE_IDLE_AFTER",
    "DEFAULT_PRESENCE_LONG_TASK_AFTER",
    "DEFAULT_PRESENCE_UPDATE_INTERVAL",
    "DEFAULT_REASONING_EFFORT",
    "DEFAULT_RICH_PRESENCE_ACTIVE_DEBOUNCE",
    "DEFAULT_RICH_PRESENCE_CONTEXT_MAX_AGE",
    "DEFAULT_RICH_PRESENCE_ENABLED",
    "DEFAULT_RICH_PRESENCE_IDLE_INTERVAL",
    "DEFAULT_RICH_PRESENCE_RECENT_IDLE_INTERVAL",
    "DEFAULT_RICH_PRESENCE_TIMEOUT",
    "DEFAULT_SELF_IMPROVEMENT",
    "DEFAULT_SELF_IMPROVEMENT_TIMEOUT",
    "ELEMENTS",
    "LABEL_TARGETS",
    "MAX_CODEX_STDIO_LIMIT",
    "MAX_NIGHTLY_RECAP_CONTEXT_MAX_CHARACTERS",
    "MIN_CODEX_STDIO_LIMIT",
    "MOOD_BASELINE_STRENGTH",
    "MOOD_DECAY_PER_MINUTE",
    "MOOD_LABELS",
    "NIGHTLY_RECAP_CONTEXT_MAX_ENV",
    "NIGHTLY_RECAP_ENV",
    "NIGHTLY_RECAP_TIMEOUT_ENV",
    "NIGHTLY_RECAP_TIMEZONE_ENV",
    "PERSONALITY_SCOPES",
    "PLACEHOLDERS",
    "QWEN_AUDIO_SEMANTIC_ENV",
    "QWEN_AUDIO_TOKEN_ENV",
    "QWEN_AUDIO_URL_ENV",
    "SELF_IMPROVEMENT_ENV",
    "SELF_IMPROVEMENT_TIMEOUT_ENV",
    "SESSION_ARCHIVE_AFTER",
    "SESSION_DELETE_AFTER",
    "SUPER_ADMIN_USERS_ENV",
    "TEXT_MODE",
    "THEIA_VERSION",
    "VOICE_FRAME_BYTES",
    "VOICE_MODE",
    "AudioOutput",
    "AudioProtocolError",
    "AudioProvider",
    "AudioProviderCapabilities",
    "AudioProviderError",
    "AudioProviderEvent",
    "AudioProviderEventCallback",
    "AudioServiceConfig",
    "CodexAppServer",
    "CodexAppServerError",
    "CodexBot",
    "CodexTransientRestartError",
    "CodexTurnCancelled",
    "CodexTurnTimeoutError",
    "CodexUpdateResult",
    "CodexUpdater",
    "CustomizationError",
    "FrontendCustomizationStore",
    "LighthouseView",
    "NightlyRecapManager",
    "OpenAICompatibleAudio",
    "PersonalityError",
    "PersonalityProfile",
    "PersonalityStore",
    "PersonalitySummary",
    "PresenceManager",
    "QwenAudioProvider",
    "RichPresenceManager",
    "SendMessage",
    "TTSConfig",
    "TheiaBot",
    "VoiceConversationSink",
    "VoiceModeError",
    "VoiceModeManager",
    "VoiceSegment",
    "VoiceSession",
    "_DecisionView",
    "_FormView",
    "_ImageResultView",
    "_JsonModal",
    "_MemoryView",
    "_MoodState",
    "_PaginatorView",
    "_PendingApproval",
    "_PromptModal",
    "_ResponseDelivery",
    "_Session",
    "_TextModal",
    "_TurnState",
    "_UsageView",
    "_UserInputView",
    "_about_embed",
    "_codex_logger",
    "_command_embed",
    "_configured_paths",
    "_configured_user_ids",
    "_credits_embed",
    "_env_bool",
    "_env_float",
    "_error_message",
    "_format_thought_duration",
    "_interaction_allows_tools",
    "_interaction_request_sender",
    "_is_always_admin_user",
    "_is_guild_install",
    "_is_server_admin",
    "_is_super_admin_user",
    "_is_tool_item",
    "_is_user_only_install",
    "_login_required_embed",
    "_maybe_create_response_thread",
    "_mention_prompt",
    "_message_context",
    "_name_new_response_thread",
    "_path_from_value",
    "_personality_summary_embed",
    "_reaction_paginators",
    "_restart_in_place",
    "_safe_approval_reason",
    "_safe_error_reason",
    "_safe_intermediate_text",
    "_safe_log_label",
    "_safe_thinking_status",
    "_should_respond_to_message",
    "_skill_entries",
    "_split_pages",
    "_subtext",
    "_theia_revision",
    "_thread_name",
    "_truncate",
    "_typing_indicator",
    "_usage_details_embed",
    "_usage_embed",
    "_user_requested_thread",
    "_verified_change_status",
    "bot",
    "canonical_target",
    "codex_about",
    "codex_approve",
    "codex_btw",
    "codex_credits",
    "codex_customize",
    "codex_deny",
    "codex_improvements",
    "codex_login",
    "codex_memory",
    "codex_mode",
    "codex_model",
    "codex_personality",
    "codex_restart",
    "codex_skill",
    "codex_stop",
    "codex_undo",
    "codex_usage",
    "customization_context",
    "customization_element_autocomplete",
    "customization_target_autocomplete",
    "display_target",
    "handle_login",
    "handle_request",
    "model_autocomplete",
    "personality_autocomplete",
    "render_lighthouse",
    "render_template",
    "send_paginated",
    "send_response",
    "session_key",
    "skill_autocomplete",
]


async def _run_secure_launcher() -> None:
    """Unlock the private credential vault before connecting to Discord."""
    bot.codex.enable_secure_credentials()
    try:
        await bot.lighthouse.start()
        await bot.lighthouse.pause_secret_input()
        try:
            await bot.codex.unlock_secure_credentials()
        finally:
            bot.lighthouse.resume_secret_input()
        token = (
            bot.codex.secure_credential("TOKEN")
            or bot.codex.secure_credential("DISCORD_TOKEN")
            or bot.codex.secure_credential("THEIA_DISCORD_TOKEN")
        )
        if not token:
            raise VaultError("The credential vault does not contain a Discord token.")
        try:
            await bot.start(token)
        finally:
            del token
    finally:
        await bot.close()


if __name__ == "__main__":
    try:
        asyncio.run(_run_secure_launcher())
    except VaultError as exc:
        raise SystemExit(str(exc)) from None
