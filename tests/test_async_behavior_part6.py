...
_ASYNC_MOOD_NAMESPACE = f"async-behavior-mood-{uuid4().hex}"


def _mood_test_key(name: str) -> str:
    return f"{_ASYNC_MOOD_NAMESPACE}:{name}"
...
class AsyncBehaviorTests(AsyncBehaviorTestBase):
    ...
    def test_mood_is_isolated_by_session_user_guild_and_channel(self) -> None:
        ...
        _apply_mood_event(
            server, server._session(source_key), "The request failed.", now=100.0
        )
    ...
    async def test_persisted_mood_restores_with_elapsed_decay(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch.dict(
                ...
                server = main.CodexAppServer()
                _apply_mood_event(
                    server, server._session("session"), "The build failed.", now=100.0
                )
